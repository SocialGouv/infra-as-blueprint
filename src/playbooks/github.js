...
module.exports = async () => {
  ...
  const playbook = async () => {
    ...
    await iterator.eachOfSeries(projects, async (projectConfig, projectName) => {
      
      const projectKubeconfigs = {}
      let kubeconfigsLastCreationDate

      await iterator.eachOf(
        clusters,
        async (clusterConfig, clusterName) => {
          const {
            environment,
          ...
          const kubeconfigCreationDate = dayjs(kubeconfigSecret.metadata.creationTimestamp).toDate();
          if(!kubeconfigsLastCreationDate || kubeconfigsLastCreationDate < kubeconfigCreationDate){
      ...
      await iterator.eachSeries(repos, async (repoConfig) => {
        ...
        await iterator.eachOfSeries(environments, async (environementConfig, envName) => {
